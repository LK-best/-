...
# Импорт API (Blueprints)
# Убедись, что файлы news_api.py и jobs_api.py лежат рядом или в папке data
# Если они в папке data, импорт будет: from data import news_api, jobs_api
from data import news_api, jobs_api  

app = Flask(__name__)
app.config["SECRET_KEY"] = "yandexlyceum_secret_key"
# Отключаем кодировку ASCII для корректного отображения кириллицы в JSON
...
@app.route("/")
def index():
    # Главная страница сразу перенаправляет на список работ
...
def add_job():
    ...
    if form.validate_on_submit():
        ...
        db_sess.commit()
        return redirect('/info')
    return render_template("register_for_the_job.html", title="Add Job", form=form)
...
def edit_department(id):
    ...
    if form.validate_on_submit():
        ...
        db_sess.commit()
        return redirect('/departments')
    return render_template('register_for_the_department.html', title='Редактирование департамента', form=form)
...
def main():
    ...
    if not db_sess.query(Jobs).filter(Jobs.id == 1).first():
        ...
        job1.categories.append(cat1) # Добавляем категорию, если объект cat1 существует
        db_sess.add(job1)
        db_sess.commit()
    ...
    if not db_sess.query(Departments).first():
        dep1 = Departments(title="Инженерный отдел", chief=1, members="2, 3", email="eng@mars.org")
        dep2 = Departments(title="Научный отдел", chief=2, members="1", email="sci@mars.org")
        db_sess.add(dep1)
        db_sess.add(dep2)
        db_sess.commit()

    # Запуск сервера (host='0.0.0.0' для доступа из локальной сети)
